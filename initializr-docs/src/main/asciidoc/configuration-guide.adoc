...
Similarly, an `ApplicationPropertiesCustomizer` can contribute properties to the
configuration files of the generated project, that is `application.properties` or
`application.yaml` depending on the requested `ConfigurationFileFormat`. Properties added
directly belong to the main source set and the default profile, `file` and `profile`
address the other source sets and Spring profiles. Each combination is written to
+src/{sourceSet}/resources/application[-{profile}]+, with the extension of the requested
format. Combinations without properties are skipped, except for the main source set and
the default profile, which is always written:
