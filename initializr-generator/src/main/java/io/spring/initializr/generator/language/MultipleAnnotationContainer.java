...
/**
 * A container for {@linkplain Annotation annotations} that supports multiple annotations.
 *
 * @author Sijun Yang
 */
public class MultipleAnnotationContainer implements AnnotationHolder {
	...
	/**
	 * Add operation is not supported for multiple annotation containers.
	...
	public void add(ClassName className, Consumer<Builder> annotation) {
		throw new UnsupportedOperationException(
				"Add operation with potential overwrite is not supported for multiple annotation containers. " +
	 ...
	 * This method always adds a new annotation, never overwrites existing ones.
	...
	public void addToList(ClassName className, Consumer<Builder> annotation) {
		List<Builder> builders = this.annotations.computeIfAbsent(className, k -> new ArrayList<>());
	...
	/**
	 * Remove the annotation at the specified index for the given class name.
	 * @param className the class name of the annotation
	 * @param index the index of the annotation to remove
	 * @return {@code true} if an annotation was removed, {@code false} otherwise
	 */
	public boolean removeAt(ClassName className, int index) {
		List<Builder> builders = this.annotations.get(className);
		if (builders != null && index >= 0 && index < builders.size()) {
			builders.remove(index);
			if (builders.isEmpty()) {
				this.annotations.remove(className);
			}
			return true;
		}
		return false;
